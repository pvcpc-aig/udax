	...
	def feed(self, raw_data, defscore=1):
		...
		for weight, direction, i, j in self._link_func(LinkMaker(self._tokens, tokens)):
			...
			try:
				...
				raise LinkError("corrupted link table, do not modify the graph outside of the PageRank interface")
			...
			if (direction & Direction.FORWARDS):
				...
				if i_has_ref != j_has_ref:
					raise LinkError("corrupted link table, do not modify graph outside of the PageRank interface")
				Icout.append(j)
				Iwout.append(weight)
				Jcin.append(i)
				Jwin.append(weight)
...
# --- Linking Algorithms -----------------------------------------------
def proxy(rad=2, weight=1, direction=Direction.FORWARDS):
